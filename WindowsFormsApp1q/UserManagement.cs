...
namespace WindowsFormsApp1
{
    public partial class UserManagement : Form
    {
        ...
        {
            InitializeComponent();
            // Подписываем события (можно сделать и через дизайнер, но здесь для надёжности)
        ...
        private void LoadUsers(string searchText = "")
        {
            try
            {
                ...
                {
                    conn.Open();
                    string sql = "SELECT ID, Логин, Роль, Заблокирован, Попытки FROM Пользователи";
                ...
                if (dialog.ShowDialog() == DialogResult.OK)
                {
                    try
                    {
                        using (OleDbConnection conn = new OleDbConnection(connectionString))
                        {
                            ...
                            string hashed = HashHelper.ComputeSha256Hash(dialog.Password);
                            string insert = "INSERT INTO Пользователи (Логин, Пароль, Роль, Заблокирован, Попытки) VALUES (?,?,?,0,0)";
        ...
        private void BtnEdit_Click(object sender, EventArgs e)
        {
            ...
            using (EditUserDialog dialog = new EditUserDialog(login, role))
            {
                if (dialog.ShowDialog() == DialogResult.OK)
                {
                    try
        ...
        private void BtnResetPassword_Click(object sender, EventArgs e)
        {
            ...
            if (MessageBox.Show($"Сбросить пароль на новый случайный пароль?\n\nОн будет показан в следующем окне.", "Подтверждение", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
            {
                try
                {
                    string hashed = HashHelper.ComputeSha256Hash(newPass);
                    using (OleDbConnection conn = new OleDbConnection(connectionString))
                    {
                        conn.Open();
                        ...
                        using (OleDbCommand cmd = new OleDbCommand(sql, conn))
                        {
                            // Для OleDb важен строгий порядок добавления параметров!
                            cmd.Parameters.AddWithValue("@pwd", hashed);
                            cmd.Parameters.AddWithValue("@id", id);
                            cmd.ExecuteNonQuery();
                        }
                    ...
                    // Выводим сгенерированный пароль, чтобы его можно было скопировать
        ...
        // Надежный метод генерации случайного пароля
